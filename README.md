...
`urban-observatory` is in early prototype-planning. The repo carries workflow rules (`AGENTS.md`), an architecture sketch (`docs/architecture.md`), the public scope, concept, methodology, v0-scope, object-model, source-strategy, source-inventory, data-dictionary, and report-outline documents in `docs/`, and a site-abstracted **worked-pattern method appendix** ([`docs/method-appendix-worked-pattern.md`](docs/method-appendix-worked-pattern.md)) demonstrating the interpretive chain on one abstracted case. **No datasets, notebooks, named-site analyses, or report artifacts exist yet** — those are candidate later phases that depend on decisions that have not been made.
