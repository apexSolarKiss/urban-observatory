...
- `_dsa-surface/` — pinned, byte-identical `surface-shell`, `surface-panel` and `surface-action` carriers, the optional `surface-shell.js` navigation runtime, and the mode-aware ASK wordmark pair, all used only by `index.html`. `surface-shell.js` is vendored because that surface **adopts** the shell's responsive navigation; the three diagram pages adopt none of it and load none of these files.
